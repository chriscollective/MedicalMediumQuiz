    ...
    <div className="min-h-screen bg-gradient-to-br from-[#FAFAF7] to-[#F7E6C3]/20 relative overflow-hidden">
      ...
      <div className="bg-white/80 backdrop-blur-lg shadow-sm border-b border-[#A8CBB7]/20">
        <div className="container mx-auto px-4 py-4 flex items-center gap-4">
          <Button onClick={onBack} variant="ghost" className="text-[#A8CBB7] hover:bg-[#F7E6C3]/20">
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        {mode === "change-password" && (
          <Card className="max-w-xl mx-auto mb-8 border-[#A8CBB7]/20">
            <CardHeader>
              ...
              <CardDescription className="text-[#636e72]">請輸入目前密碼與新密碼</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {message && <div className="text-sm text-[#2d3436]">{message}</div>}
              <div>
                <label className="block text-sm text-[#2d3436] mb-1">目前密碼</label>
                <input type="password" className="w-full border rounded-md px-3 py-2 border-[#A8CBB7]/40 focus:outline-none focus:ring-2 focus:ring-[#A8CBB7]" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} />
              </div>
              <div>
                <label className="block text-sm text-[#2d3436] mb-1">新密碼</label>
                <input type="password" className="w-full border rounded-md px-3 py-2 border-[#A8CBB7]/40 focus:outline-none focus:ring-2 focus:ring-[#A8CBB7]" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} />
              </div>
              <div>
                <label className="block text-sm text-[#2d3436] mb-1">確認新密碼</label>
                <input type="password" className="w-full border rounded-md px-3 py-2 border-[#A8CBB7]/40 focus:outline-none focus:ring-2 focus:ring-[#A8CBB7]" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
              </div>
              <div className="flex items-center gap-2">
                <Button variant="outline" className="border-[#A8CBB7] text-[#2d3436]" disabled={loading} onClick={() => { setMode(null); setCurrentPassword(""); setNewPassword(""); setConfirmPassword(""); setMessage(null); }}>取消</Button>
                <Button className="bg-gradient-to-r from-[#A8CBB7] to-[#9fb8a8] text-white" disabled={loading} onClick={async () => {
                  try {
                    setMessage(null);
                    if (!currentPassword || !newPassword || !confirmPassword) { setMessage('請完整填寫'); return; }
                    if (newPassword.length < 6) { setMessage('新密碼至少 6 碼'); return; }
                    if (newPassword !== confirmPassword) { setMessage('兩次新密碼不一致'); return; }
                    if (newPassword === currentPassword) { setMessage('新密碼不可與目前密碼相同'); return; }
                    setLoading(true);
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {items.map((it, idx) => (
            <motion.div key={it.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: idx * 0.1 }}>
              <Card className="border-[#A8CBB7]/20 overflow-hidden">
                <div className={`h-1 bg-gradient-to-r ${it.color}`} />
                <CardHeader>
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${it.color} flex items-center justify-center mb-3`}>{it.icon}</div>
                  <CardTitle className="text-[#2d3436]">{it.title}</CardTitle>
                  <CardDescription className="text-[#636e72]">{it.description}</CardDescription>
                </CardHeader>
                <CardContent>
                  <Button variant="ghost" className="text-[#A8CBB7] hover:bg-[#F7E6C3]/30 w-full" onClick={it.action}>開啟</Button>
                </CardContent>
              </Card>
            </motion.div>
          ))}
